...
st.title("PC - Nielsen Discover Data Analysis")
...
with st.sidebar:
    ...
    if st.button("Upload data"): # 저장된 경우에만 활성화
        st.session_state["uploaded_df"] = load_df_from_db(db_name="PC.db", db_folder=db_folder_name)
        ...
        try:
            ...
            if st.button("Save the pre-loaded data"):
                if save_df_to_db(st.session_state["uploaded_df"].copy(), db_name="PC.db", db_folder=db_folder_name, fill_value=fill_na_value):
...
if st.session_state["final_data_rows"] is not None:
    st.session_state["scope"] = st.radio("Select the level to analyze.", ["market", "manufacturer", "manufacturer/segment", "manufacturer/brand", "manufacturer/subbrand"], index=["market", "manufacturer", "manufacturer/segment", "manufacturer/brand", "manufacturer/subbrand"].index(st.session_state["scope"]))
    ...
    market_df = st.session_state["final_data_rows"][nan_condition]
    ...
    if st.session_state["scope"] == "market":
        ...
        all_manufacturer_df = st.session_state["final_data_rows"][nan_condition]
    ...
    elif st.session_state["scope"] == "manufacturer/subbrand":
        cols_to_check = st.session_state["final_data_rows"].columns[6:7]
        # 선택된 컬럼들이 모두 NaN인 조건으로 행 발췌
        nan_condition = st.session_state["final_data_rows"][cols_to_check].isnull().all(axis=1)
